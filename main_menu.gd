...
@onready var bg_panel = $Background  # Make sure this matches your node name
...
@export var color_a := Color(0.3, 0.8, 0.0)
@export var color_b := Color(0.6, 0.0, 0.1)
@export var color_c := Color(0.9, 0.4, 0.2)
@export var bg_intensity := 1.5  # increase this for stronger color pulse
...
# Find Spectrum Analyzer on the Music bus
...
func _process(_delta: float) -> void:
	if analyzer:
		...
		color_phase += _delta * (1.0 + current_amplitude * 8.0)
		...
		# Panels use style overrides; ColorRect uses modulate
